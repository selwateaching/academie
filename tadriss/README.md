# TADRISS — version SaaS (comptes, essai 14 jours, IA réelle)

Application web pour enseignants algériens : préparation de séances, contrôles,
cahier journal, avec assistant IA (Claude) et export PDF/Word.
...
## 2. Mettre TADRISS en ligne pour vos abonnés

Vous n'avez pas encore d'hébergement dédié à cette version : le plus simple
pour démarrer et comparer avec votre application existante (tadriss.fr) est un
hébergeur gratuit pour commencer, par exemple **Render** :

1. Créez un compte sur [render.com](https://render.com) (gratuit).
2. "New +" → "Web Service" → connectez ce dépôt GitHub, dossier `tadriss/`.
...
5. Déployez. Render vous donne une adresse temporaire du type
   `https://tadriss-xxxx.onrender.com` — c'est cette adresse que vous
   partagerez à vos abonnés pour tester, en attendant de brancher un vrai
   nom de domaine.

**Important — stockage des données** : par défaut, TADRISS utilise un fichier
SQLite (`tadriss.db`). Sur la plupart des hébergeurs gratuits, ce fichier est
...
3. C'est tout : l'assistant IA de TADRISS (fiches, contrôles, cours, cahier
...
pouvez passer à un modèle moins coûteux avec `TADRISS_AI_MODEL=claude-sonnet-5`
...
├── static/
│   ├── app.js            # Interface TADRISS (connectée au backend)
