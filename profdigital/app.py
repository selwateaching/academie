...
def seed_demo_classe(user):
    ...
    classe = Classe(
        user_id=user.id, nom="6e B", matiere="Mathématiques", annee_scolaire=DEFAULT_STATE["year"]
    )
...
def safe_name(s):
    ...
    return s or "profdigital_document"
...
<title>{html.escape(c.get("title", "ProfDigital"))}</title>
...
<p>{prompt_text}</p>{sections}<hr><small>Généré par ProfDigital</small></body></html>"""
...
def make_docx_doc(c, path):
    ...
    for a, b in c.get("sections", []):
        d.add_heading(a, 1)
        d.add_paragraph(b)
    d.add_paragraph("Généré par ProfDigital")
    d.save(path)
