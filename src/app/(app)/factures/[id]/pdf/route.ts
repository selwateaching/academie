...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const element: any = React.createElement(DocumentPdf, {
    ...
    statusLabel: (FACTURE_STATUSES as any)[facture.status]?.label,
    date: facture.date,
    secondaryDate: { label: 'Échéance', value: facture.dueDate },
    company: facture.company,
    client: facture.client,
    chantierName: facture.chantier?.name,
    lines: facture.lines,
    discountPct: facture.discountPct,
    totals,
